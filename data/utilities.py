...
def get_sig_path_DP_kernel_matrix(X_train, X_test, order, sigma_0: int=1.0):
    '''
    Computes the DotProduct Kernel
    '''
    n_train = X_train.shape[0]
    n_test = X_test.shape[0]
    X = np.concatenate([X_train, X_test], axis=0)
    assert X.shape == (n_train + n_test, X_train.shape[1])

    path_sig_repre = []
    x_axis = np.asarray(range(X.shape[1])).reshape(-1,1)
    for x in tqdm(X):
        two_dim_stream = np.append(x_axis, x.reshape(-1,1), axis=1)
        path_sig_repre.append(ts.stream2sig(two_dim_stream, order))

    dp_kernel = DotProduct(sigma_0=sigma_0)
    X = np.asarray(path_sig_repre)
    K = dp_kernel(X)
    assert K.shape == (n_train+n_test, n_train+n_test)
    return K[:n_train, :n_train], K[n_train:, :n_train]
